...
Pour un dégré de liberté de 9 on a $t_9 = \frac{0,10 - 0}{0,03944} = 2,5355$. En lisant sur la table de $t$ ou en utilisant R on peut donc lire la p-value.

```{r}
2*pt(q=2.5355, df=9, lower.tail=FALSE)
```
...
Une étude mesure la taille de coquillages provenant de 2 lacs différents. L'objectif est de savoir si la taille des coquillages de ces deux populations est égale ou différente.
